...
type Interface interface {
	...
	ReconcileKind(ctx context.Context, o *v1.Secret) pkgreconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.Secret.
type Finalizer interface {
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.Secret) pkgreconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.Secret if they want to process resources for which
...
type doReconcile func(ctx context.Context, o *v1.Secret) pkgreconciler.Event
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client kubernetes.Interface, lister corev1.SecretLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
	...
	if _, ok := r.(pkgreconciler.LeaderAware); ok {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.Secrets(s.namespace)
	...
	if errors.IsNotFound(err) {
		...
		if del, ok := r.reconciler.(pkgreconciler.OnDeletionInterface); ok {
	...
	case pkgreconciler.DoFinalizeKind:
	...
	case pkgreconciler.DoObserveKind:
	...
	if reconcileEvent != nil {
		...
		if pkgreconciler.EventAs(reconcileEvent, &event) {
			...
			if _, isEvent := reconcileEvent.(*pkgreconciler.ReconcilerEvent); !isEvent {
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.Secret, desiredFinalizers sets.String) (*v1.Secret, error) {
	...
	patcher := r.Client.CoreV1().Secrets(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.Secret) (*v1.Secret, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.Secret, reconcileEvent pkgreconciler.Event) (*v1.Secret, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
...
func (s *state) reconcileMethodFor(o *v1.Secret) (string, doReconcile) {
	if o.GetDeletionTimestamp().IsZero() {
		if s.isLeader {
			return pkgreconciler.DoReconcileKind, s.reconciler.ReconcileKind
		} else if s.isROI {
			return pkgreconciler.DoObserveKind, s.roi.ObserveKind
		}
	} else if fin, ok := s.reconciler.(Finalizer); s.isLeader && ok {
		return pkgreconciler.DoFinalizeKind, fin.FinalizeKind
	}
	return "unknown", nil
}